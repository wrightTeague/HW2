...
A C++ TimeCode class that represents an amount of time as hours, minutes, and seconds (for example 4:15:32). It stores only one value, t, the total number of seconds (4:15:32 is stored as t = 15332).
...
- The constructor and arithmetic roll over out-of-range values, so TimeCode(0, 61, 0) becomes 1:1:0.
...
Hours have no upper limit. ToString() uses h:m:s with no zero-padding (for example 8:5:9). The math library (cmath) is not used.
